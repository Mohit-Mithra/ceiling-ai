...
class Trainer():
    ...
    def __init__(self, opt, train_loader, val_loader, model, executor):
        ...
        self.optimizer = tf.keras.optimizers.Adam(lr=opt.learning_rate)
        #self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.seq2seq.parameters()), lr=opt.learning_rate)
        optimizer = optimizer.minimize(filter(lambda p: p.requires_grad, model.seq2seq.parameters()))
    ...
    def train(self):
        ...
        while t < self.num_iters:
            ...
            for x, y, ans, idx in self.train_loader:
                ...
                self.optimizer.zero_grad()              #Check tf practise
                if self.reinforce:
                ...
                self.optimizer.run()                   #Check tf practise
    ...
    def _to_numpy(self, x):
        return x.data.cpu().numpy()             #Check tf practise