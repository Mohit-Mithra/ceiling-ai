...
class Decoder(BaseRNN):
    ...
    def __init__(self, vocab_size, max_len, word_vec_dim, hidden_size,
        ...
        self.end_id = end_id
        self.embedding = keras.layers.Embedding(self.output_size, self.word_vec_dim)
        self.rnn = self.rnn_cell(self.word_vec_dim, self.hidden_size, n_layers, batch_first=True, dropout=dropout_p)
        self.out_linear = keras.layers.Dense(self.hidden_size, self.output_size)
        if use_attention:
            self.attention = Attention(self.hidden_size)

    def forward_step(self,input_var,hidden,enconder_outputs):
        ...
        predicted_softmax = keras.layers.Softmax(output.view(batch_size, output_size, -1), 2)
    ...
    def forward_sample(self, encoder_outputs, encoder_hidden, reinforce_sample=False):
        ...
        decoder_hidden = self._init_state(encoder_hidden)    
        decoder_input = Variable(torch.LongTensor(batch_size, 1).fill_(self.start_id)) # don't know what this start_id is.
        if use_cuda:
    ...
    def decode(i, output, reinforce_sample=reinforce_sample):
        ...
        if reinforce_sample:
            symbols = tf.random.categorical(np.exp(output.reshape(batch_size, -1)),1) # ********just check this line
        else:
            #symbols = output.topk(1)[1].view(batch_size, -1)
            symbols= np.amax(output[1]).resahpe(batch_size,-1)   #***********some shitty logic, doesn't make sense, need to change
    ...
    def _cat_directions(self, h):
        if self.bidirectional_encoder:
            h = np.concatenate(([h[0:h.size(0):2], h[1:h.size(0):2]], axis=2))
        return h